    ...
    return (
        <Nav defaultActiveKey="/" className="sidebar-options flex-md-column">
            <div className="nav-brand-cont">
                <Navbar.Brand> <Link className="brand" to="" onClick={() => {
                    clickHandler("")
                }}>
                    <div style={{display:"flex", justifyContent:"center", margin:"10px"}}>
                    <img className="sidebar-icon" src={Images.icon} alt="sidebar icon" width="80px" height="80px"/>
                    </div>
                    <div>
                    Handyheart.
                    </div>
            ...
            <Nav className="flex-md-column nav-item-cont">
                ...
                {listOfPages.map((page, index) => {
                    if (selectedPage === page.href){
                        return (
                            <Link className="page" style={{color:"black"}} to={page.href} onClick={() => {
                        ...
                        return (
                            <Link className="page" to={page.href} onClick={() => {
                                clickHandler(page.href)
                            }}>{page.name}</Link>);
                    }
